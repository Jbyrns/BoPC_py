# -*- coding: utf-8 -*-
"""
Created on Tue Jul 17 09:10:46 2018

@author: jeffr
"""

import numpy as np
import sample_sigma2
import sample_A
import sample_T_const


class Bayesian_Unmixing_3D:
    

    @classmethod
    def Bayesian_Unmixing_3D(obj, event):
        ...
        m_compt = 1;
        print('MONTE CARLO ITERATIONS')
        
        while m_compt<Nmc and run_simulation
        
            set(handles(28),'string',int2str(m_compt)); """ make a handle dictionary"""
            """ sampling noise variance"""
            sigma2_est = sample_sigma2.sample_sigma2.sample_sigma2(A_est,M_est,Y); 
            
            """ sampling abundance vectors """
            A_est = sample_A.sample_A.sample_A(Y',M_est',A_est',R,P,sigma2_est)';
            A_est = A_est/(sum(A_est).T@np.ones((1,R)))';
            
            % sampling endmember projections
            T_est, M_est =sample_T_const.sample_T_const.sample_T_const(A_est,M_est,T_est,sigma2_est,Tsigma2r,matU,Y_bar,Y,endm_proj,bool_plot,y_proj);  
